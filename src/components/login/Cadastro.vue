<template>
    <div class="fundo-cadastro" v-if="exibir">
        <div class="cadastro" >
                ...
                <input type="text" :class="`form-control ${invalidaCadastro[0]}`" placeholder="nome de usuário" id="nome" name="nome" v-model="name">
                <input type="email" :class="`form-control ${invalidaCadastro[1]}`"  placeholder="Email" id="email" name="email" v-model="emailCadastro">
                <input type="password" :class="`form-control ${invalidaCadastro[2]}`"  id="password" placeholder="senha" name="password" v-model="passwordCadastro">
                <input type="password" :class="`form-control ${invalidaCadastro[3]}`"  id="password2" placeholder="confirma senha" name="password2" v-model="confirmPassword">
                <button type="submit" @click="cadastro" class="buttonCadastro">Cadastro</button>
        </div>
    </div>
</template>
<script>
    ...
    methods:{
        ...
        async cadastro(){
            try{
                ...
                if(response.data.nome === this.name ){
                    this.name = "";
                    this.emailCadastro="";
